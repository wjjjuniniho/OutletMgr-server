...
import tongji.sse.outletmanager.database.util.AuthenticationStatusObjectDBHelper;
...
import tongji.sse.outletmanager.servlet.util.AuthenticationStatusHelper;
...
public class AuthenticationServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
				...
				for (NameValuePair queryParam : queryParamList) {
					if (queryParam.getName().equals(ServletConstant.STOREID)) {
						storeId = queryParam.getValue();
							...
							ServletConstant.USERNAME)) {
						username = queryParam.getValue();
					} else if (queryParam.getName().equals(
							ServletConstant.PASSWORD)) {
						password = queryParam.getValue();
					}
				}

				if (storeId != null && !storeId.equals("") && username != null
						&& !username.equals("") && password != null
						&& !password.equals("")) {
					AuthenticationStatusObject authenticationStatusObject = AuthenticationStatusObjectDBHelper
							.getAuthenticationStatusObject(storeId, username,
									password);
					if (authenticationStatusObject.isSuccessful()) {
						authenticationStatusJSONObject = AuthenticationStatusHelper
								.toJSONObject(authenticationStatusObject);
		...
		out = response.getWriter();
		if (authenticationStatusJSONObject != null) {
			out.write(authenticationStatusJSONObject.toString());
		}
		out.flush();
		out.close();
	}
